...
merge 1:1 gvkey year using "$data/fluidity.dta", keepus(prodmktfluid) keep(1 3) nogen
...
keep if year==$year11 | year==$year22 | year==$year33 | year==$year44
...
*/ cashnorm marketshare_naics tfpr lernerindex tobinq fluid logtm logtm2 tmoversale logtm_3yr logtm_3yr2 tmoversale_3yr /*
...
gen d_`var'=`var'$year22 - `var'$year11
gen d_`var'_pre = `var'$year44 -`var'$year33
}
...
*/ d_fluid d_logtm d_logtm2 d_tmoversale d_logtm_3yr d_logtm_3yr2 d_tmoversale_3yr /*
...
gen dcashnorm2010=cashnorm$year11 - cashnorm2010
gen dlogsale2010=logsale$year11 - logsale2010
gen dmarkup2010=markup$year11 - markup2010
gen drdnorm2010=rdnorm$year11 - rdnorm2010
gen dlogemp2010=logemp$year11 - logemp2010

global control3 dlogsale2010 dlogemp2010

reghdfe d_logemp d_share_narrowai $control $control2  $control3 [aw=weight] if include == 1, a($fevar) cluster($clustervar)
keep if e(sample)==1
...
replace year=2020
merge 1:1 gvkey year using "$data/compustat_20192020", keep(1 3) nogen
replace d_logsale=log(sale)-logsale$year11
replace d_logemp=log(emp)-logemp$year11
...
esttab using "$table/original/table_ols_splitperiod_`data'.tex", replace label f booktabs style(tex) alignment (c) nofloat ///
...
star(* 0.10 ** 0.05 *** 0.01)  ///
mgroups("$\Delta$ Log Sales" "$\Delta$ Log Employment", pattern(1 0 1 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})  ) 
