...
gen s="Manu" if sector>=31 & sector<=33
replace s="Retail" if sector==44 | sector==45 | sector==42
replace s="Finance" if sector==52
*replace s="Health" if sector==62
replace s="Other" if s==""
...
foreach sec in "Manu" "Retail" "Finance" "Other" {

gen `var'_s`sec' = `var' if s=="`sec'"

egen 	`var'_temp = std(`var'_s`sec')
drop 	`var'_s`sec'
rename 	`var'_temp `var'_s`sec'

replace `var'=`var'_s`sec' if s=="`sec'"
...
eststo:  reghdfe `Y' `X'   $control $control2 [aw=weight] if include == 1 & s=="`sec'", a($fevar) cluster($clustervar)
estadd local naics2_fe "Y"
estadd local control "Y"
}
	
esttab using "$table/original/table_ols_`data'_bysector_norm.tex", replace label f booktabs style(tex) alignment (c) nofloat ///
