...
if "`data'"=="bg" {
merge 1:1 gvkey year using "$data/bg_compustat_allai", keepus(share_allai njob njob_narrowai) keep(1 3) nogen
...
ren share_allai share_narrowai2010
...
if "`data'"=="bg" {
merge 1:1 gvkey year using "$data/bg_compustat_allai", keepus(share_allai) keep(1 3) nogen
merge 1:1 gvkey year using "$cognismdata", keepus(aiemp) keep(3) nogen
}
...
ren share_allai share_narrowai2018
...
ren share_allai share_narrowai2014
...
esttab using "$table/original/table_ols_weighted_`data'contall.tex", replace label f booktabs style(tex) alignment (c) nofloat ///
