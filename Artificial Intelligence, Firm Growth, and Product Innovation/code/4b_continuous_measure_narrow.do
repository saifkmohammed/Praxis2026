...
global varlist markup markup2 logemp emp sale logsale roa ros logsaleperworker sga_norm logsga logcogs logxopr logrd rdnorm fluid selffluid /*
...
if "`data'"=="bg" {
merge 1:1 gvkey year using "$data/bg_compustat_allai", keepus(share_narrowai njob njob_narrowai) keep(1 3) nogen
...
ren share_narrowai share_narrowai2010
...
if "`data'"=="bg" {
merge 1:1 gvkey year using "$data/bg_compustat_allai", keepus(share_narrowai) keep(1 3) nogen
merge 1:1 gvkey year using "$cognismdata", keepus(aiemp) keep(3) nogen
}
...
ren share_narrowai share_narrowai2018
...
ren share_narrowai share_narrowai2014
...
*/d_logsaleperworker d_logsga d_logcogs d_logxopr d_logrd d_rdnorm d_tfpr markup2010 d_logmv
...
lab var d_markup "$\Delta$ Markup (De Loecker)"
lab var d_markup2 "$\Delta$ Markup (Traina)"
...
esttab using "$table/original/table_ols_weighted_`data'cont.tex", replace label f booktabs style(tex) alignment (c) nofloat ///
