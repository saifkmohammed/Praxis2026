...
global varlist markup markup2 logemp emp sale logsale roa ros logsaleperworker sga_norm logsga logcogs logxopr logrd rdnorm fluid selffluid cashnorm marketshare_naics tfpr lernerindex tobinq logtm logtm2 tmoversale logtm_3yr logtm_3yr2 tmoversale_3yr logprodpat1_fyear logprodpat2_fyear logprodpat1_iyear logprodpat2_iyear logprocesspat1_fyear logprocesspat2_fyear logprocesspat1_iyear logprocesspat2_iyear logmv logprodpat1_fyear_wc logprodpat2_fyear_wc logprodpat1_fyear_wv logprodpat2_fyear_wv age
...
merge 1:1 gvkey using "$data/crsp_return", keep(1 3) nogen


***************************************************
********Table A9: drop top 1% of size distribution*
****************************************************

sum logsale2010, d
gen byte top1size = (logsale2010>=r(p99))
gen byte top5size = (logsale2010>=r(p95))

eststo clear
foreach Y in d_logsale d_logemp d_logmv {
eststo:  reghdfe `Y' d_share_narrowai  [aw=weight] if include == 1 & top1size==0 , a($fevar) cluster($clustervar)
estadd local naics2_fe "Y"
estadd local control "N"
eststo:  reghdfe `Y' d_share_narrowai $control $control2 [aw=weight] if include == 1 & top1size==0 , a($fevar) cluster($clustervar)
estadd local naics2_fe "Y"
estadd local control "Y"
}
	
esttab using "$table/original/table_ols_cognism_droptop1size.tex", replace label f booktabs style(tex) alignment (c) nofloat ///
...
eststo clear
foreach Y in d_logsale d_logemp d_logmv {
eststo:  reghdfe `Y' d_share_narrowai  [aw=weight] if include == 1 & top5size==0 , a($fevar) cluster($clustervar)
estadd local naics2_fe "Y"
estadd local control "N"
eststo:  reghdfe `Y' d_share_narrowai $control $control2 [aw=weight] if include == 1 & top5size==0 , a($fevar) cluster($clustervar)
estadd local naics2_fe "Y"
estadd local control "Y"
}
	
esttab using "$table/original/table_ols_cognism_droptop5size.tex", replace label f booktabs style(tex) alignment (c) nofloat ///
noobs nodepvars nomtitles ///
hlinechar(`=char(151)') ///
varwidth(30) modelwidth (12) collabels(none)  legend  ///
cells(b(star fmt(3)) se(par fmt(3)))   /// 
...
foreach Y in return return_capm return_ff3 return_ff4 {
...
esttab using "$table/original/table_return.tex", replace label f booktabs style(tex) alignment (c) nofloat ///
noobs nodepvars nomtitles ///
hlinechar(`=char(151)') ///
varwidth(30) modelwidth (12) collabels(none)  legend  ///
cells(b(star fmt(3)) se(par fmt(3)))   /// 
...
stats(naics2_fe control r2_a N , fmt(0 0 %9.3fc %9.0fc ) labels("Industry FE" "Controls" "Adj R-Squared" "Observations") ) ///
star(* 0.10 ** 0.05 *** 0.01)  ///
mgroups("Buy-and-hold Return" "CAPM-adjusted Return" "FF3-adjusted Return" "FF4-adjusted Return", pattern(1 0 1 0 1 0 1 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})  ) 
